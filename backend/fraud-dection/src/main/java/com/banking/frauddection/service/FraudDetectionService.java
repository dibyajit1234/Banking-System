...
public class FraudDetectionService {
    ...
    public void checkTransaction(Map<String, Object> payload) {
        ...
        FraudCheckResult result = performFraudCheck(accountNumber,new BigDecimal(amount),senderBalance);
    ...
    private FraudCheckResult performFraudCheck(
        ...
        if(isVelocityExceeded(accountNumber))return new FraudCheckResult(true,"To many transactions in 60 seconds - velocity limit exceeded");
    ...
    private boolean isVelocityExceeded(String accountNumber){
        ...
        if(count != null && count==1) redisTemplate.expire(key,60, TimeUnit.SECONDS);
        log.info("Velocity check-> accountNumber: {}, count:{}",accountNumber,count);
